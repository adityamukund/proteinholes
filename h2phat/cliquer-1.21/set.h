...
#ifndef ELEMENTSIZE
typedef unsigned long long int setelement;
# if (ULONG_MAX == 65535)
 ...
 */
UNUSED_FUNCTION INLINE
static set_t set_resize(set_t s, uint size) {
	uint n;
...
static set_t set_duplicate(set_t s) {
	...
	tnew=set_new(SET_MAX_SIZE(s));
	memcpy(tnew,s,SET_ARRAY_LENGTH(s)*sizeof(setelement));
	return tnew;
}
 ...
 */
UNUSED_FUNCTION INLINE
static int set_return_next(set_t s, uint n) {
	if (n<0)
		n=0;
	else
		n++;
