...
DescendNode* Cover::DescendList::newDescendNode(CoverNode* covernode, 
  DLPtrListNode<CoverNode>* current_child, REAL dist) {
  return new(descendnode_seglist.getPtr()) 
    DescendNode(covernode,current_child,dist);
}
...
// for building cover with threads
...
void* g(void* p) {
  ...
  while(enlargedata->getRemaining()>0) {
    while(1) {
      ...
      if(counter<=0) 
	break;
      PointLevelParent* plp=enlargedata->getPointLevelParent(counter-1);
      if(plp) {
	descend_list.reset(pcover,plp->getPoint());
	pcover->insert(descend_list,plp,tempradius,templevel,parg);
	enlargedata->incrThreadNCallsToGetDist(
	  tid, descend_list.getNCallsToGetDist());
      }
    }
...
int Cover::setPoints(Points& points,ThreadsWithCounter* threads,
		     EnlargeData* enlargedata) {
  const Point* point=0;
  int counter=0;
  //int BLOCKSIZE=threads->getBlockSize();
...
Cover::Cover(const Point* point,SegList<DLPtrListNode<CoverNode> >& seglist,
	     int inumlevels, int iminlevel) :
...
/*
Cover::Cover(const Point* point,SegList<DLPtrListNode<CoverNode> >& seglist,
	     int inumlevels,int iminlevel,int counter,
	     vector<PointLevelParent>& vplp,CoverNode* covernodes) :
  DLPtrList<CoverNode>(&seglist),
  root(0),
  //theta(rtheta),
  //mu(1.0/(1.0-rtheta)),
  ...
  minlevel(iminlevel),
  maxlevel(iminlevel+inumlevels-1),
  //threadncallstogetdist(0),
  //mergecallstogetdist(0),
  number_inserted(0),
  number_deep(0),
  number_duplicates(0),
  ...
  //dup_seglist(1024)
{
  ...
  int templevel;for(int i=0;i<counter;i++) {
	...
	maxradius=tempradius;
      ...
      //ncallstogetdist+=descend_list.getNCallsToGetDist();
  }
}
*/

void Cover::enlargeBy(EnlargeData& enlargedata,Points& points) {

  ThreadsWithCounter* threads=enlargedata.getThreads();
  int NTHREADS=threads->getNThreads();
  //Points& points=enlargedata.getPoints();
  if((NTHREADS>0)&&(points.getRemaining()>0)) {
    ...
    //EnlargeData* enlargedata=new EnlargeData(BLOCKSIZE,points.getRemaining());
    ...
    //enlargedata.setRemaining(points.getRemaining());
    ...
    while(enlargedata.getRemaining()>0) {
      ...
      merge(threads,counter,enlargedata);
  ...
  } else {  // we're serial or building mergecover
...
void Cover::merge(ThreadsWithCounter* threads,int counter,EnlargeData& enlargedata) {
...
void Cover::insert(DescendList& descend_list,PointLevelParent* plp,
		   REAL& tempradius, int& templevel,EnlargeArg* parg) {
  const Point* p=descend_list.getPoint();

  REAL dist_to_root=descend_list.getDistToRoot();

  if(dist_to_root>=mu*maxradius) { //will adjust root
    ::setMaxRadiusAndMinLevel(dist_to_root,theta,tempradius,templevel);
    new(plp) PointLevelParent(p,templevel+1,root);
    return;
  }
  

  if(dist_to_root==0.0) {
    new(plp) PointLevelParent(p,INT_MAX,root);
    return ;
  }

  //dist_to_root>0 at this point

  int m=minlevel+numlevels-1;
  //since dist<mu*maxradius S_minlevel is nonempty, will enter
  while((descend_list.first()) 
	&& (descend_list.getLevel()<m)) { 
    CoverNode* q=descend_list.descend();
    if(q) { //duplicate
      new(plp) PointLevelParent(p,INT_MAX,q);
      return;
    }
  } 
  ...
  if(descend_list.getCount()!=0) { // too deep
    ...
    new(plp) PointLevelParent(p,m+1,near_node);
  } else { // not too deep
    CoverNode* parent=descend_list.getParent(); 
    if(parent) {
      int new_level=descend_list.getRLevel()+1;
      new(plp) PointLevelParent(p,new_level,parent);
	...
	new(plp) PointLevelParent(p,level+1,root);
      }
    }
  }
  return;
...
Cover::DescendList::DescendList() :
  ...
  dist_to_root(0.0),
  ncallstogetdist(0),
  descendnode_seglist(1024)
{
}

void Cover::DescendList::reset(const Cover* cover,const Point* ppoint,
  REAL r) {
  ...
  ncallstogetdist=1;
  if(dist_to_root<mu*radius+r) {
    DescendNode* descendnode=new(descendnode_seglist.getPtr())
      DescendNode(root,root->getChildren()->first(),dist_to_root);
    append(descendnode);
  }
}
...
CoverIndices::CoverIndices(const Cover* cover,const Points* points,int* base) :
  sz(sizeof(REAL)),
  mytheta(theta),
...
Cover::Cover(Points& points,SegList<DLPtrListNode<CoverNode> >& seglist,
	     const CoverIndices& coverindices) :
  DLPtrList<CoverNode>(&seglist),
  root(0),
