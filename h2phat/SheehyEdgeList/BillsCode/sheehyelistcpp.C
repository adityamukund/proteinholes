...
//TO COMPILE, RUN:
//g++ -g -o sheehyelistcpp -DFLOAT sheehyelistcpp.C ThreadsWithCounter.C IDLList.C IDLListNode.C Vector.C Cover.C Timer.C Point.C CoverNode.C EnlargeData.C -lpthread
...
int* makeCoverTree(REAL* X, int N, int d, REAL theta, int numlevels, int minlevel, int NTHREADS, int BLOCKSIZE, int *rootLevel, REAL** radii) {
    mu=1.0/(1.0-theta);
    dim=d;
    Vectors vectors(X,N);
