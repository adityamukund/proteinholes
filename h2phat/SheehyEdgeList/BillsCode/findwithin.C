...
/* Get dimensions of second field of input */
  ...
  /* The third argument */
  //the third argument is  withinradius, numfindlevels
  int nfields = mxGetNumberOfFields(prhs[2]);
  if(nfields!=2) {
    mexErrMsgTxt("Third argument should have two fields.");
  }

  //tmp=mxGetField(prhs[2],0,fnames_in[0]);
  tmp=mxGetField(prhs[2],0,within_in[0]);
  if(!mxIsClass(tmp,"double")) {
    mexErrMsgTxt("First field of third argument must be double\n");
  }
  mwSize ndims_radius=mxGetNumberOfDimensions(tmp);
  const mwSize* dims_radius=mxGetDimensions(tmp);
  if(ndims_radius!=2) {
    mexErrMsgTxt("First field of third argument should have 2 dims\n");
  }
  if(dims_radius[1]!=1) {
    mexErrMsgTxt("First field of third argument should have dims[1]=1\n");
  }

  double* pwithinradius=(double*)mxGetData(tmp); 
  double withinradius=*pwithinradius;

  tmp=mxGetField(prhs[2],0,within_in[1]);
  if(!mxIsClass(tmp,"int32")) {
    mexErrMsgTxt("Third field of fourth argument must be int32\n");
  }
  mwSize ndims_findlevels=mxGetNumberOfDimensions(tmp);
  const mwSize* dims_findlevels=mxGetDimensions(tmp);
  if(ndims_findlevels!=2) {
    mexErrMsgTxt("Second field of third argument should have 2 dims\n");
  }
  if(dims_findlevels[1]!=1) {
    mexErrMsgTxt("Second field of third argument should have dims[1]=1\n");
  }

  int* pnumfindlevels=(int*)mxGetData(tmp); 
  int numfindlevels=*pnumfindlevels;

  if(dims_radius[1]!=dims_findlevels[1]) {
    mexErrMsgTxt("Size mismatch in third argument\n");
  }
  ...
  if(!mxIsClass(tmp,"double")) {
    mexErrMsgTxt("Fourth argument should be double\n");
  }
  ...
  if(dims_radius[1]!=1) {
    if((dims_radius[1]!=NY)||(dims_findlevels[1]!=NY)) {
      mexErrMsgTxt("Mismatch between third and fourth arguments\n");
    }
  }
  
  
  double* pr=pwithinradius;
  int* pf=pnumfindlevels;
  ...
  int totalfound=0;
  if(dims_radius[0]==1) {
    FindWithinData findwithindata(&threads,vectorsY,*pr,*pf,descendlists);
  ...
  bool test=true;
  for(int i=0;i<NY;i++) {
    test=cover.checkFindWithin(findvectors.getPoint(i),r[i],numfindlevels[i],descendlists[i]);
    //cout << "i=" << i << endl;
    if(test==false) {
      cout << i << " failed" << endl;
    }
...
/* Create matrix for the return argument. */
  ...
  double* distances=(double*)mxGetData(fout);
  mxSetField(plhs[0],0,fnames_out[2], fout);

  flattenDescendLists(totalfound,indices,distances,NY,pi,vectorsX,descendlists);
