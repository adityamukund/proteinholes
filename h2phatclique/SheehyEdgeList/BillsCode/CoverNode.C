...
CoverNode::CoverNode(const Point* ppoint,
		     int ilevel,
		     CoverNode* pparent,
		     SegList<DLPtrListNode<CoverNode> >* child_seglist) :
...
void CoverNode::printOn(ostream& os) const {
  os << "covernode " << " level=" << level << endl;
    ...
    os << endl;
    for(DLPtrListNode<CoverNode>* childnode=children.first();
	  childnode;childnode=children.next(childnode)) {
